...
export default function Dashboard({ user }: DashboardProps) {
  ...
  const [ignoredAlertIds, setIgnoredAlertIds] = useState<string[]>(() => {
    try {
      const stored = localStorage.getItem(`ignored_urgent_actions_${user.uid}`);
  ...
  const saveIgnoredAlerts = (ids: string[]) => {
    setIgnoredAlertIds(ids);
    try {
      localStorage.setItem(`ignored_urgent_actions_${user.uid}`, JSON.stringify(ids));
    } catch (e) {
      console.error('Error saving ignored alerts:', e);
    }
  };

  const handleIgnoreAction = (e: React.MouseEvent, actionId: string, actionTitle?: string) => {
    e.stopPropagation();
    if (!ignoredAlertIds.includes(actionId)) {
      const updated = [...ignoredAlertIds, actionId];
      saveIgnoredAlerts(updated);
      toast.success(actionTitle ? `تم تجاهل تنبيه "${actionTitle}"` : 'تم تجاهل التنبيه بنجاح');
    }
  };

  const handleIgnoreAll = () => {
    const activeIds = urgentActions.map(a => a.id);
    const updated = Array.from(new Set([...ignoredAlertIds, ...activeIds]));
    saveIgnoredAlerts(updated);
    toast.success('تم تجاهل جميع التنبيهات العاجلة');
  };

  const handleRestoreAction = (actionId: string) => {
    const updated = ignoredAlertIds.filter(id => id !== actionId);
    saveIgnoredAlerts(updated);
    toast.success('تمت استعادة التنبيه بنجاح');
  };

  const handleRestoreAll = () => {
    saveIgnoredAlerts([]);
    toast.success('تمت استعادة كافة التنبيهات المتجاهلة');
    ...
    <div className="space-y-8 rtl" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {ignoredAlertIds.length > 0 && (
            <button
              onClick={() => setShowIgnoredModal(true)}
              className="px-3.5 py-2 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200/80 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 shadow-sm"
              title="عرض وإدارة التنبيهات العاجلة التي تم تجاهلها"
            >
              <EyeOff className="w-4 h-4 text-amber-600" />
              <span>تنبيهات متجاهلة ({ignoredAlertIds.length})</span>
            </button>
          )}
      ...
      {urgentActions.filter(a => !ignoredAlertIds.includes(a.id)).length > 0 && (
        <motion.div 
        ...
        >
          ...
          <div className="relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-red-100 rounded-2xl flex items-center justify-center text-red-600 animate-pulse shrink-0">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="bg-red-600 text-white text-[10px] font-black px-2 py-0.5 rounded-full">
                    {urgentActions.filter(a => !ignoredAlertIds.includes(a.id)).length}
                  </span>
                </div>
                <p className="text-sm font-bold text-red-700/80 mt-0.5">
                  لديك {urgentActions.filter(a => !ignoredAlertIds.includes(a.id)).length} إجراءات معلقة تتطلب التدخل الفوري
                </p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
              {urgentActions.filter(a => !ignoredAlertIds.includes(a.id)).map((action) => (
                <div
                  key={action.id}
                  className="bg-white border border-red-200/80 rounded-xl overflow-hidden shadow-sm flex items-center transition-all hover:border-red-400 group/pill"
                >
                  <button
                    onClick={() => navigate(action.link)}
                    className="px-3 py-2 text-[11px] font-black text-red-700 hover:bg-red-50 transition-all flex items-center gap-1.5"
                  ...
                  <button
                    onClick={(e) => handleIgnoreAction(e, action.id, action.title)}
                    className="px-2 py-2 text-slate-400 hover:text-red-700 hover:bg-red-100 border-r border-red-100 transition-all"
              ...
              {omittedSessions.length > 0 && (
                <button
                  ...
                  className="bg-red-600 text-white px-5 py-2 rounded-xl text-xs font-black shadow-lg shadow-red-100 hover:bg-red-700 transition-all flex items-center gap-1.5 shrink-0"
                >
              ...
              <button
                ...
                className="bg-white hover:bg-red-100/60 border border-red-200 text-red-700 px-3.5 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 shadow-sm shrink-0"
                title="تجاهل كافة التنبيهات المعروضة حالياً"
              >
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-slate-900">التنبيهات العاجلة المتجاهلة</h3>
                    <p className="text-xs text-slate-500 font-medium">قائمة التنبيهات التي قمت بصرف النظر عنها أو تجاهلها</p>
                  </div>
                </div>
                <button
              ...
              <div className="max-h-72 overflow-y-auto space-y-3 pr-1">
                {ignoredAlertIds.length === 0 ? (
                  <div className="py-10 text-center text-slate-400">
                    ...
                    <p className="text-sm font-bold">لا توجد تنبيهات متجاهلة حالياً</p>
                  </div>
                ) : (
                  ignoredAlertIds.map((id) => {
                    const matchedAction = urgentActions.find(a => a.id === id);
                    return (
                      <div
                        key={id}
                        className="bg-slate-50 border border-slate-200/70 p-3.5 rounded-2xl flex items-center justify-between gap-3"
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-xs font-black text-slate-800 truncate">
                              {matchedAction?.title || `تنبيه معرف: ${id}`}
                            </p>
                            <p className="text-[10px] text-slate-500 font-medium truncate">
                              {matchedAction?.subtitle || 'تم تجاهله من اللوحة'}
                            </p>
                          </div>
                        </div>
                        <button
                          onClick={() => handleRestoreAction(id)}
                          className="bg-white hover:bg-indigo-50 text-indigo-600 border border-slate-200 hover:border-indigo-200 px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 shrink-0 shadow-xs"
                          title="استعادة هذا التنبيه ليظهر مجدداً"
                        >
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                {ignoredAlertIds.length > 0 && (
                  <button
                    ...
                    className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2.5 rounded-xl text-xs font-black transition-all flex items-center gap-1.5"
                  >
                ...
                <button
                  onClick={() => setShowIgnoredModal(false)}
                  className="mr-auto bg-slate-900 text-white px-5 py-2.5 rounded-xl text-xs font-black hover:bg-slate-800 transition-all"
                >
                  إغلاق
                </button>
              </div>
