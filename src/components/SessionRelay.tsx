    ...
    try {
      ...
      if (!isJudgment) {
        ...
        await addDoc(collection(db, 'tasks'), {
          ...
          description: `تم إنشاء مهمة تلقائية لمتابعة واستلام الحكم الصادر في الجلسة. منطوق الحكم: ${judgmentResult || decision}`,
          assignedTo: selectedSession.lawyerId || user.uid,
    ...
    try {
      ...
      if (addSessionDecision) {
        if (!addSessionIsJudgment) {
          ...
          await addDoc(collection(db, 'tasks'), {
            title: 'استلام الحكم والتمثيل القضائي',
            description: `تم إنشاء مهمة تلقائية لمتابعة واستلام الحكم الصادر. منطوق الحكم: ${addSessionJudgmentResult || addSessionDecision}`,
            assignedTo: user.uid,
            dueDate: new Date(Date.now() + 10 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
            status: 'pending',
            priority: 'high',
    ...
    try {
      ...
      if (editSessionDecision && editSessionIsJudgment) {
        ...
        if (judgmentsSnap.empty) {
          ...
          await addDoc(collection(db, 'tasks'), {
            title: 'استلام الحكم والتمثيل القضائي',
            description: `تم إنشاء مهمة تلقائية لمتابعة واستلام الحكم الصادر (بعد تعديل الجلسة). منطوق الحكم: ${editSessionJudgmentResult || editSessionDecision}`,
            assignedTo: selectedSession.lawyerId || user.uid,
            dueDate: new Date(Date.now() + 10 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
            status: 'pending',
            priority: 'high',
          ...
          <table className="w-full text-right border-collapse print:border-t-0">
            ...
            <tbody className="divide-y divide-slate-100 print:divide-y-0">
                        ...
                        {session.decision ? (
                          ...
                          <>
                            <button
                              ...
                              className="flex items-center gap-2 text-emerald-700 font-extrabold text-xs hover:bg-emerald-100 bg-emerald-50 px-5 py-2.5 rounded-xl border border-emerald-200 print:hidden w-fit shadow-sm transition-all hover:scale-[1.02]"
                            >
                        ...
                        <div className="flex items-center gap-3 mt-1 print:hidden">
                          {session.decision && (
                            <button
                              ...
                              className="flex items-center gap-1.5 text-[10px] font-black text-slate-600 hover:text-slate-800 hover:bg-slate-100 px-2 py-1 rounded-lg transition-all border border-slate-200/50 bg-slate-50"
                            >
                              <CheckCircle2 className="w-3.5 h-3.5 text-slate-500" />
                              تعديل حضور وقرار الجلسة
                            </button>
                          )}
                          <button
                            onClick={() => {
              ...
              {filteredSessions.length === 0 && (activeTab === 'omitted' ? omittedExpertSessions.length === 0 : filteredExpertSessions.length === 0) && (
                <tr>
                  <td colSpan={activeTab === 'omitted' ? 8 : 7} className="p-20 text-center text-slate-400 font-bold">
                    {activeTab === 'omitted' ? (
                      <div className="flex flex-col items-center justify-center gap-3 text-emerald-600">
                        <CheckCircle className="w-12 h-12 text-emerald-500" />
                        <span className="text-base font-black text-slate-900">النظام آمن بالكامل</span>
                        <span className="text-xs font-bold text-slate-500">لا توجد أي جلسات معلقة أو فائتة لم يتم ترحيل قراراتها</span>
                      </div>
                    ) : (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                <h2 className="text-xl font-black text-slate-900 font-sans">حضور وإثبات قرار الجلسة</h2>
              ...
              <div className="p-8 space-y-6">
                {selectedSession && (
                  <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-4 space-y-1.5 text-xs text-slate-700">
                    <div className="flex justify-between items-center">
                      <span className="font-extrabold text-emerald-950">تفاصيل جلسة الحضور المقروءة:</span>
                      <span className="bg-emerald-100 text-emerald-800 px-2.5 py-0.5 rounded-full font-black text-[10px]">
                        {selectedSession.date ? selectedSession.date.split('T')[0] : '---'}
                      </span>
                    </div>
                    <div className="font-bold text-slate-600 mt-1">
                      <p>رقم القضية: <span className="text-slate-900 font-black">{selectedSession.caseInfo?.caseNumber || '---'} / {selectedSession.caseInfo?.year || '---'}</span></p>
                      <p className="mt-1">الموكل: <span className="text-slate-900 font-black">{selectedSession.caseInfo?.clientName || '---'}</span></p>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700">القرار أو الإجراء المراد ترحيله</label>
                  <textarea
                    required
                    rows={3}
                    placeholder="اكتب القرار المطلوب ترحيله، مثل: التأجيل للاطلاع أو لتقديم مستندات..."
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleEditSession} className="p-8 space-y-6 overflow-y-auto flex-1">
                <div className="bg-amber-50/70 border border-amber-100 rounded-2xl p-4 space-y-1.5 text-xs text-amber-950 font-bold">
                  <div className="flex justify-between items-center">
                    <span>تفاصيل الجلسة المقروءة:</span>
                    <span className="bg-amber-100/90 text-amber-900 px-2.5 py-0.5 rounded-full font-black text-[10px]">
                      {selectedSession.date ? selectedSession.date.split('T')[0] : '---'}
                    </span>
                  </div>
                  <p>رقم القضية: {selectedSession.caseInfo?.caseNumber || '---'} / {selectedSession.caseInfo?.year || '---'}</p>
                  <p>الموكل: {selectedSession.caseInfo?.clientName || '---'}</p>
                </div>

                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700">تعديل القرار أو الإجراء المراد ترحيله</label>
                  <textarea
                    required
